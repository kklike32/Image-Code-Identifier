...
# Load image
image  = cv2.imread('handCode4.png')
...
cv2.imwrite('greenLines.png', image_rgb)


def extract_boxes(image_path, lower_green=np.array([35, 100, 50]), upper_green=np.array([85, 255, 255]), black_thresh=50):
    # Load the image
    ...
    for i, contour in enumerate(contours):
        ...
        if aspect_ratio >= 0.9 and aspect_ratio <= 1.1:
            ...
            if np.any(thresh_box):
                cv2.imwrite(f'box_{i}.png', box)
                
                
# Provide the path to your image
image_path = 'greenLines.png'

# Call the function to extract boxes
extract_boxes(image_path)